...
| 修改 | 群管理员 / 群主 | 否 | 群聊 | 使用本地头像/名称关键字立即修改当前群资料 |
...
| 使用本地存储项 | 群管理员 / 群主 | 否 | 群聊 | 按序号立即使用当前群的本地头像或名称资源 |
...
修改头像 https://example.com/avatar_list.txt && ./avatars
修改名称 新群名
修改名称 name_list.txt
修改名称 name_list.txt && 备用群名
定时修改头像 0 8 * * * https://example.com/avatar.jpg
定时修改头像 0 8 * * * https://example.com/avatar_list.txt
定时修改头像 0 8 * * * https://example.com/avatar_list.txt && ./avatars
定时修改名称 0 8 * * * 新群名
定时修改名称 0 8 * * * name_list.txt
定时修改名称 0 8 * * * name_list.txt && 备用群名
上传
取消
随机头像
随机名称
...
- 推荐使用英文文件名，例如 `avatar_list.txt` 和 `name_list.txt`
- 多个资源来源可使用 `&&` 连接，例如 `avatar_list.txt && ./avatars` 或 `name_list.txt && 备用名称`
- `本地头像` 和 `本地名称` 可作为关键字直接调用已上传的本地资源池
...
- 发送 `修改 本地头像` 或 `修改 本地名称` 会立即从本地资源池调用对应资源
- 发送 `使用本地存储项 头像 3` 或 `使用本地存储项 名称 2` 可按编号精确调用本地资源
- 发送 `本地存储列表` 会显示本群头像/名称资源数量摘要
- 发送 `本地存储列表 头像 1` 或 `本地存储列表 名称 1` 可分页查看具体内容
- 列表中的编号为全局序号，可直接配合 `删除本地存储项 头像 3` 或 `删除本地存储项 名称 2` 使用
